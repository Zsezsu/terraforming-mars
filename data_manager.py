...
def execute_select(statement, fetchall=True):
    """
...
def execute_insert(statement, fetchone=False):
    """
    Execute INSERT INTO statement parameterized.
    """
    with establish_connection() as conn:
...
def execute_update(statement):
    """
    Execute UPDATE statement parameterized.
    """
    with establish_connection() as conn:
        with conn.cursor() as cursor:
            cursor.execute(statement)
